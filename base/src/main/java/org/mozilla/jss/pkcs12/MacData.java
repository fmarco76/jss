...
public class MacData implements ASN1Value {
     ...
     *  entity that created the HMAC possessed the symmetric key. The default
     *  generator algorithm is SHA1.
     *
            ...
            DigestException, TokenException, CharConversionException
    {
        ...
        try {
            ...
            System.out.println("Select the digest algo: "+algID.getOID().toDottedString());
            // generate key from password and salt
            KeyGenerator kg = token.getKeyGenerator(KeyGenAlgorithm.SHA256_MARCO);
            kg.setCharToByteConverter(new PasswordConverter());
            kg.initialize(params);
            SymmetricKey key = kg.generate();

            // perform the digesting
            JSSMessageDigest digest = token.getDigestContext(HMACAlgorithm.SHA2562);
            System.out.println("Digest algo is: "+ HMACAlgorithm.SHA256.toOID().toDottedString());
            digest.initHMAC(key);
            byte[] digestBytes = digest.digest(toBeMACed);
            System.out.println("Byto to compute the mac: " + Utils.HexEncode(toBeMACed));

            // put everything into a DigestInfo
            this.mac = new DigestInfo(new AlgorithmIdentifier(DigestAlgorithm.SHA256.toOID()), new OCTET_STRING(digestBytes));
